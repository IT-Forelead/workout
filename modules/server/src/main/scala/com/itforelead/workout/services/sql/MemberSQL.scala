...
object MemberSQL {
  ...
  def selectMemberFilter(
    ...
    val res: AppliedFragment =
      sql"""SELECT * FROM members WHERE user_id = $userId AND deleted = false #$filterBy""".apply(
        id
      )
  ...
  val changeActiveTimeSql: Query[LocalDateTime ~ MemberId, Member] =
    sql"""UPDATE members SET active_time = $timestamp WHERE id = $memberId RETURNING *""".query(
      decoder
    )

  val currentMemberActiveTimeSql: Query[MemberId, LocalDateTime] =
    sql"""SELECT active_time FROM members WHERE id = $memberId AND deleted = false""".query(
      timestamp
    )

  val selectExpiredMembers: Query[Void, Member] =
    sql"""SELECT * FROM members WHERE DATE(active_time) = DATE(NOW() + INTERVAL '3 DAY')""".query(
      decoder
    )
