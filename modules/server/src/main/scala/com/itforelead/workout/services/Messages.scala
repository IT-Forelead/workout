...
object Messages {
  def apply[F[_]: GenUUID: Sync](redis: RedisClient[F], messageBroker: MessageBroker[F], users: Users[F])(implicit
      ...
      override def sendValidationCode(userId: Option[UserId] = None, phone: Tel): F[Unit] = {
        Random.scalaUtilRandom[F].flatMap { implicit random =>
          for {
            ...
            _ <- createAndSendMessage(userId, phone, validationCode, messageText, now, DeliveryStatus.SENT)
          } yield ()
        }
      }
