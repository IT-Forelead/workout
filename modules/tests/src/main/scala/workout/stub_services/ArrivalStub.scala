...
import com.itforelead.workout.domain.Arrival.{ArrivalFilter, ArrivalWithMember, CreateArrival}
...
class ArrivalStub[F[_]] extends ArrivalService[F] {
  ...
  override def getArrivalWithTotal(userId: UserId, filter: ArrivalFilter, page: Int): F[Arrival.ArrivalWithTotal] = ???
